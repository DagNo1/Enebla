...
//        database = FirebaseDatabase.getInstance().getReference("Users")
        ...
        binding.suCaNextButton.setOnClickListener {
            showProgressBar()
            val formFilled = valueFiled(binding.fnInputInCa, binding.tv3InCa) + valueFiled(binding.eInputInCa, binding.tv6InCa) +
                    valueFiled(binding.pInputInCa, binding.tv7InCa)

            if (formFilled != 6) {//all 6 fields are filled
                Toast.makeText(this@SignUp, "* fields are required to register.", Toast.LENGTH_LONG).show()
                hideProgressBar()
...
//            registerUser()
    ...
    private fun valueFiled(v: View, t : TextView): Int{//checks values were filed and marks unfilled ones
        ...
        when (v) {
            ...
            binding.eInputInCa ->
                if(!str(binding.eInputInCa).trim().matches(emailR)) filled = 1
            binding.pInputInCa ->
